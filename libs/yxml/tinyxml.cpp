...
Modified:
   Added an optional parameter to delete the node after removing it from the childrens
#endif
bool TiXmlNode::RemoveChild( TiXmlNode* removeThis, bool del )
{
...
void TiXmlElement::StreamOut( TIXML_OSTREAM * stream, bool unclosed ) const
{
	(*stream) << "<" << value;
...
void TiXmlDocument::StreamOut( TIXML_OSTREAM * out, bool unclosed ) const
{
	const TiXmlNode* node;
	for ( node=FirstChild(); node; node=node->NextSibling() )
	{
...
void TiXmlAttribute::StreamOut( TIXML_OSTREAM * stream, bool unclosed ) const
{
	if (value.find( '\"' ) != TIXML_STRING::npos)
	{
...
void TiXmlComment::StreamOut( TIXML_OSTREAM * stream, bool unclosed ) const
{
	(*stream) << "<!--";
	//PutString( value, stream );
	(*stream) << value;
...
void TiXmlText::StreamOut( TIXML_OSTREAM * stream, bool unclosed ) const
{
	if ( cdata )
	{
		(*stream) << "<![CDATA[" << value << "]]>";
...
void TiXmlDeclaration::StreamOut( TIXML_OSTREAM * stream, bool unclosed ) const
{
	(*stream) << "<?xml ";
...
void TiXmlUnknown::StreamOut( TIXML_OSTREAM * stream, bool unclosed ) const
{
